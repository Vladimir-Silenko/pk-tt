...
let Data = [
    {
        id: 0,
        price: 600,
        name: 'Тарелка керамическая ручной работы  ⌀ 12',
        newItem: false,
        popular: false,
        star: null,
        prevPrice: 810,
    ...
    },
    {
        id: 1,
        price: 0,
        name: 'Кашпо для цветов керамическое  СИМПЛГРЕЙ',
        newItem: false,
        popular: false,
        star: null,
        prevPrice: null,
        photo: '../assets/images/kashpo.jpg',
        count: 0,
    },
    {
        id: 2,
        price: 12999,
        name: 'Умные часы SMARTWATCH XM 109 черный',
        newItem: false,
        popular: true,
        prevPrice: null,
        photo: '../assets/images/watches.png',
        count: 1,
    },
    {
        id: 3,
        price: 15700,
        name: 'Золотое кольцо МАЛИКА с аметистом и фианитами',
        newItem: true,
        popular: false,
        prevPrice: null,
        photo: '../assets/images/ring.png',
        count: 0,
    },
    {
        id: 4,
...
Data = Data.map(({ price, name, photo, prevPrice, popular, newItem, id, count }) => {
    a += ` <li style="position-relative" class="card">
    ...
    const buttons = document.querySelectorAll('.count-block__btn');

    buttons.forEach((button) => {
        button.addEventListener('click', (event) => {
            const id = event.target.id;
            const countElement = document.querySelector(`.count[id="${id}"]`)
